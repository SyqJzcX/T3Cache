...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        elif (current['type'] == 'Erta-Cache') or (current['type'] == 'Delta-Cache-pre'):
        ...
        **kwargs, ##
        ...
        if current['compute']:
            ...
            for i, block in enumerate(self.double_blocks):
                ...
                # 第2层，保存pre状态
                if i == 1 and cache_dic['Delta-DiT-pre']:
                    delta_base_img = img.clone()
                    delta_base_txt = txt.clone()
                # 第12层，决定复用/计算
                if cache_dic['Delta-DiT-pre'] and i == 11:
                    if current['type'] == 'Delta-Cache-pre':
                        img = delta_base_img + cache_dic['Delta-Cache-img']
                        txt = delta_base_txt + cache_dic['Delta-Cache-txt']
                    else:
                        cache_dic['Delta-Cache-img'] = img - delta_base_img
                        cache_dic['Delta-Cache-txt'] = txt - delta_base_txt
